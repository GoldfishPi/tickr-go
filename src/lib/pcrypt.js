...
var pcrypt = {
    ...
    from_utf8: function (str) {
        return decodeURIComponent(escape(str));
    },

    pbkdf2: function (password, salt, iterations, size_bytes) {
        var passbits = sjcl.codec.utf8String.toBits(pcrypt.to_utf8(password));
        var key = sjcl.misc.pbkdf2(passbits, salt, iterations, size_bytes * 8, function (p) {
    ...
    gen_auth: function (username, password) {
    ...
    derivePassword(password_str, salt_str) {
    ...
    createPassword( password, options) {
        var iter = 24000;
